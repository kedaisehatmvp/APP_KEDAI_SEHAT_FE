...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize carousel with faster autoplay
    initCarousel();
...
// Initialize carousel with faster autoplay (3 seconds)
function initCarousel() {
    const carousel = document.getElementById('promoCarousel');
    if (carousel) {
        const carouselInstance = new bootstrap.Carousel(carousel, {
            interval: 3000, // Change slide every 3 seconds (lebih cepat)
            wrap: true,
            pause: false // Auto slide terus meski dihover
        });
        
        // Add click event to all carousel items for tracking
        const carouselItems = carousel.querySelectorAll('.carousel-item a');
        carouselItems.forEach(item => {
            item.addEventListener('click', function(e) {
                const slideIndex = Array.from(this.closest('.carousel-item').parentNode.children).indexOf(this.closest('.carousel-item'));
                console.log(`Carousel slide ${slideIndex + 1} clicked`);
...
function setupEventListeners() {
    ...
    window.addToCart = function(button) {
        const menuCard = button.closest('.best-seller-card');
        const menuTitle = menuCard.querySelector('.best-seller-title').textContent;
        const menuPrice = menuCard.querySelector('.best-seller-price').textContent;
        const menuImage = menuCard.querySelector('.best-seller-img').src;
